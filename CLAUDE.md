...
```
Account names use underscores to match rcfm's account naming convention. `value` is current market value, read directly from the latest Schwab-reported positions snapshot (no history computation, so this runs fast). Pipe to `jq` to inspect, or consume via `subprocess.run(["./roi.py", "export"])` from rcfm.
